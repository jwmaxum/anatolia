    ...
    <footer className="bg-[#060608] text-stone-400 border-t border-stone-800/80 pt-16 pb-12 font-light">
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        <div className="grid grid-cols-1 md:grid-cols-12 gap-12 pb-16 border-b border-stone-800/50">
          ...
          <div className="md:col-span-4 space-y-4">
            ...
            <p className="text-xs text-stone-400 leading-relaxed max-w-sm">
              {t('hero_subtitle', 'Global leader in premium porcelain tiles, natural stones, and sintered stone slabs.')}
            </p>
            <div className="pt-2">
              <LanguageSelector />
            </div>
            <div className="flex space-x-3 pt-2">
              <a
                ...
                target="_blank"
                rel="noreferrer"
                className="w-8 h-8 rounded-full bg-stone-900 border border-stone-800 flex items-center justify-center text-stone-400 hover:text-[#c5a880] hover:border-[#c5a880] transition-colors text-xs font-semibold"
          ...
          <div className="md:col-span-8 grid grid-cols-2 sm:grid-cols-3 gap-8">
            {menus.map((group) => (
              <div key={group.id} className="space-y-3">
                <h4 className="text-xs uppercase font-mono tracking-widest text-[#c5a880] font-semibold">
                  {group.title}
                </h4>
                    ...
                    {group.children.map((child) => (
                      <li key={child.id}>
                        ...
                        >
                          ...
                          <ArrowUpRight
                            size={12}
                            className="ml-1 opacity-0 group-hover:opacity-100 transition-opacity text-[#c5a880]"
                          />
                        </Link>
                      </li>
                    ))}
        ...
        <div className="pt-8 flex flex-col sm:flex-row justify-between items-center text-xs text-stone-500 gap-4 font-light">
          <p>© {new Date().getFullYear()} {t('copyright', 'Anatolia Tile & Stone Inc. All rights reserved.')}</p>
          <div className="flex space-x-6 text-[11px]">
            ...
            <Link href="/admin" className="text-[#c5a880] hover:underline font-medium">Admin Studio</Link>
          </div>
        </div>
